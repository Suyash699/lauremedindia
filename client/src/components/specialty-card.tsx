...
export default function SpecialtyCard({ specialty }: SpecialtyCardProps) {
  return (
    <Link href={`/products?specialty=${encodeURIComponent(specialty.name)}`}>
      <Card className="group cursor-pointer hover:shadow-md transition-shadow" data-testid={`card-specialty-${specialty.id}`}>
        <CardContent className="p-0">
          ...
          <div className="p-3">
            <h3 className="text-center font-semibold text-gray-700 group-hover:text-organic-green transition-colors text-sm" data-testid={`text-specialty-name-${specialty.id}`}>
              {specialty.name}
            </h3>
          </div>
        </CardContent>
